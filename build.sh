...
if [ "$OPTION" = 'help' ]; then
    ...
    build() {
        ...
        if [ "$OPTION" = "clean" ]; then
            ...
            test -f "windows/bin/release/rive.lib" && echo 'lib linked 1'
            echo ATTEMPT IT WINDOWS
            which ar
            test -f "C:\ProgramData\Chocolatey\lib\mingw\tools\install\mingw64\x86_64-w64-mingw32\bin\ar.exe" && echo 'ok we exist'
            echo LINK IT
            ...
            test -f "windows/bin/release/rive.lib" && echo 'lib linked 2'
        else
            verbose=1 make
        fi
    }
