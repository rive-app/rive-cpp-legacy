...
namespace Diligent
{
	...
	bool GLFWDemo::CreateWindow(const char* Title,
	                            int Width,
	                            int Height,
	                            int GlfwApiHint)
	{
		...
		glfwSetWindowSizeLimits(
		    m_Window, 320, 240, GLFW_DONT_CARE, GLFW_DONT_CARE);
		return true;
	}
	...
	bool GLFWDemo::InitEngine(RENDER_DEVICE_TYPE DevType)
	{
		...
		switch (DevType)
		{
			...
			case RENDER_DEVICE_TYPE_GL:
			{
				...
				pFactoryOpenGL->CreateDeviceAndSwapChainGL(EngineCI,
				                                           &m_pDevice,
				                                           &m_pImmediateContext,
				                                           SCDesc,
				                                           &m_pSwapChain);
			...
			case RENDER_DEVICE_TYPE_VULKAN:
			{
				...
				pFactoryVk->CreateDeviceAndContextsVk(
				    EngineCI, &m_pDevice, &m_pImmediateContext);
				pFactoryVk->CreateSwapChainVk(m_pDevice,
				                              m_pImmediateContext,
				                              SCDesc,
				                              Window,
				                              &m_pSwapChain);
			...
			case RENDER_DEVICE_TYPE_METAL:
			{
				...
				pFactoryMtl->CreateDeviceAndContextsMtl(
				    EngineCI, &m_pDevice, &m_pImmediateContext);
				pFactoryMtl->CreateSwapChainMtl(m_pDevice,
				                                m_pImmediateContext,
				                                SCDesc,
				                                Window,
				                                &m_pSwapChain);
		...
		if (m_pDevice == nullptr || m_pImmediateContext == nullptr ||
		    m_pSwapChain == nullptr)
			return false;
	...
	void GLFWDemo::GLFW_ResizeCallback(GLFWwindow* wnd, int w, int h)
	{
		...
		if (pSelf->m_pSwapChain != nullptr)
			pSelf->m_pSwapChain->Resize(static_cast<Uint32>(w),
			                            static_cast<Uint32>(h));
	}

	void
	GLFWDemo::GLFW_KeyCallback(GLFWwindow* wnd, int key, int, int state, int)
	{
		auto* pSelf = static_cast<GLFWDemo*>(glfwGetWindowUserPointer(wnd));
		pSelf->OnKeyEvent(static_cast<Key>(key), static_cast<KeyState>(state));
	}

	void GLFWDemo::GLFW_MouseButtonCallback(GLFWwindow* wnd,
	                                        int button,
	                                        int state,
	                                        int)
	{
		auto* pSelf = static_cast<GLFWDemo*>(glfwGetWindowUserPointer(wnd));
		pSelf->OnKeyEvent(static_cast<Key>(button),
		                  static_cast<KeyState>(state));
	}

	void
	GLFWDemo::GLFW_CursorPosCallback(GLFWwindow* wnd, double xpos, double ypos)
	{
		float xscale = 1;
		float yscale = 1;
		glfwGetWindowContentScale(wnd, &xscale, &yscale);
		auto* pSelf = static_cast<GLFWDemo*>(glfwGetWindowUserPointer(wnd));
		pSelf->MouseEvent(float2(static_cast<float>(xpos * xscale),
		                         static_cast<float>(ypos * yscale)));
	}

	void
	GLFWDemo::GLFW_MouseWheelCallback(GLFWwindow* wnd, double dx, double dy)
	{
	}
	...
	void GLFWDemo::Loop()
	{
		...
		for (;;)
		{
			...
			for (auto KeyIter = m_ActiveKeys.begin();
			     KeyIter != m_ActiveKeys.end();)
			{
				KeyEvent(KeyIter->key, KeyIter->state);

				// GLFW does not send 'Repeat' state again, we have to keep
				// these keys until the 'Release' is received.
			...
			const auto dt =
			    std::chrono::duration_cast<TSeconds>(time - m_LastUpdate)
			        .count();
			m_LastUpdate = time;
	...
	bool GLFWDemo::ProcessCommandLine(const char* CmdLine,
...
#define _stricmp strcasecmp
		...
		if (pos != nullptr)
		{
...
#else
				std::cerr << "Direct3D11 is not supported. Please select "
				             "another device type";
				return false;
#endif
			}
...
#else
				std::cerr << "Direct3D12 is not supported. Please select "
				             "another device type";
				return false;
#endif
			}
...
#else
				std::cerr << "OpenGL is not supported. Please select another "
				             "device type";
				return false;
#endif
			}
...
#else
				std::cerr << "Vulkan is not supported. Please select another "
				             "device type";
				return false;
#endif
			}
...
#else
				std::cerr << "Metal is not supported. Please select another "
				             "device type";
				return false;
#endif
			}
			...
			{
				std::cerr << "Unknown device type. Only the following types "
				             "are supported: D3D11, D3D12, GL, VK";
				return false;
			}
		}
		else
